...
namespace Looxid.Link
{
    ...
    public class LooxidLinkData : MonoBehaviour
    {
        ...
        private const int MAX_SAMPLES = int.MaxValue;
        ...
        private List<EEGRawSignal> eegRawSignalList;
        ...
        void OnEnable()
        {
            ...
            eegRawSignalList = new List<EEGRawSignal>();
        ...
        public double GetMindAttentionData() {
            return mindIndexList[0].attention;
        }
        ...
        void OnNetworkReceiveEEGRawSignals(EEGRawSignal eegRawSignals)
        {
            ...
            eegRawSignalList.Insert(0, eegRawSignals);
            if (eegRawSignalList.Count > MAX_SAMPLES)
            {
                eegRawSignalList.RemoveAt(eegRawSignalList.Count - 1);
            }            
        }

        public List<EEGRawSignal> GetEEGRawSignalData(float seconds)
        {   

            double nowTime = LooxidLinkUtility.GetUTCTimestamp();
            List<EEGRawSignal> recentEEGRawSignalDataList = new List<EEGRawSignal>();
            for (int i = 0; i < eegRawSignalList.Count; i++)
            {
                if (nowTime - eegRawSignalList[i].timestamp <= seconds)
                {
                    recentEEGRawSignalDataList.Add(eegRawSignalList[i]);
                }
            }
            return recentEEGRawSignalDataList;
        }

        

        public double GetRawTimestamp(){
            if (eegRawSignalList.Count != 0)
                return eegRawSignalList[0].rawSignal[0].timestamp;
            else
                return -1;
        }

        public double GetIndexTimestamp(){
            if (mindIndexList.Count != 0)
                return mindIndexList[0].timestamp;
            else
                return -1;
        }

        public double GetFeatureTimestamp(){
            if (eegFeatureIndexList.Count != 0)
                return eegFeatureIndexList[0].timestamp;
            else
                return -1;
        }
